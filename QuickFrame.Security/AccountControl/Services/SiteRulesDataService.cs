...
namespace QuickFrame.Security.AccountControl.Services {
	...
	public class SiteRulesDataService : ISiteRulesDataService {
		...
		public IEnumerable<SiteRule> GetSiteRulesForUser(string userId) {
			foreach(var obj in _context.SiteRules.Where(obj => obj.IsDeleted == false && obj.UserRules.Any(user => user.UserId == userId))) {
				((IObjectContextAdapter)_context).ObjectContext.Detach(obj);
				yield return obj;
			}
		}
